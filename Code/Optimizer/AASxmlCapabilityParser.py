                                ...
                                if property_sets_value is not None and "https://admin-shell.io/idta/CapabilityDescription/PropertySet/1/0" in property_sets_value.text:
                                    for property_container in property_sets.findall(".//aas:submodelElementCollection", ns):
                                        ...
                                        if property_type_range is not None:
                                            ...
                                            prop_entry = {
                                                'property_name': get_text_or_none(prop_name),
                                                'property_comment': get_text_or_none(prop_comment),
                                                'property_ID': get_text_or_none(prop_id),
                                                'property_unit': get_text_or_none(unit),
                                                'valueType': get_text_or_none(vtype),
                                                'valueMin': get_text_or_none(min_val),
                                                'valueMax': get_text_or_none(max_val),
                                                'propertyRealizedBy': get_text_or_none(prop_relBy),
                                                'property_constraint': []
                                            }
